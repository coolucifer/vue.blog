<template>
  <div id="blog" class="shadowbox">
    <ul class="pager">
      <li class="previous">
        <router-link to="/2017-12-12">上一篇
          <span aria-hidden="true">&larr;</span>
        </router-link>
      </li>
      <li class="next">
        <router-link to="/2018-01-28">下一篇
    ...
    <div class="blog-content">
      ...
      <p>做游戏嘛,首先要搞明白规则,百度上说2048是一位叫Gabriele Cirulli的大佬写的,便去github上顺藤摸瓜找到了项目主页:<a href="http://gabrielecirulli.github.io/2048/">2048</a>.玩了一会觉得这种游戏可能不适合自己,最高玩到1024就死掉了...</p>
      <p>2048的游戏规则就是可以通过方向键/按钮/<del>念力</del>来让所有方块向同一方向移动,如果遇到相同的方块就把两个方块上的数字相加生成一个新方块,每次移动/相加都会在随机空位置生成一个值为2的方块.</p>
...
var moveLeft = function () {  //左移
  for (var i = 0; i < len; i++) {
    for (var j = 1; j < len; j++) {  //从[0][1]开始遍历数组,因为是左移,最左面一行不用检查
      ...
      <p>核心代码的第二部分就是在随机位置生成方块2,这里采用的方法有两种:<br> 1.利用<code>Math.random()</code>方法生成两个随机数(i,j)作为随机位置的坐标[i][j],检查该位置是否为0,为0则[i][j]=2,否则调用自身重新生成随机数i,j.这种方法就是逻辑上比较简单,但是明显能看出还是有很多缺点,比如若数组已满,要判断至少16次才能确定,以及判断数组已满的方法又是两次循环.所以这里采用第二种方法生成随机坐标.</p>
      <pre>
...
import ImgMoveLeft from '@/assets/images/blog-use/17-12-23-moveleft.png'
